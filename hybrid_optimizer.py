...
class HybridOptimizer:
    ...
    def __init__(
        ...
        # 融合策略参数
        acquisition_threshold: float = 0.1,  # 采集函数差值阈值
        ...
        Args:
            ...
            acquisition_threshold: 采集函数差值阈值（默认 0.1）
            random_seed: 随机种子
        """
    ...
    def add_prior_experiments(self, experiments: List[ExperimentResult]) -> None:
        ...
        for exp in experiments:
            prior_exp = PriorExperiment(
                parameters=exp.parameters,
                metrics=exp.metrics,
                metadata=exp.metadata
            )
            prior_experiments.append(prior_exp)
        
        self.llm_agent.prior_experiments = prior_experiments
    ...
    def _get_gp_best_acquisition_value(self) -> Tuple[Dict[str, Any], float]:
        """
        获取 GP 推荐的最佳点及其采集函数值
        ...
        try:
            ...
            return best_parameters, best_acquisition_value
    ...
    def suggest_parameters(
        ...
        gp_best_params, gp_best_acq_value = self._get_gp_best_acquisition_value()
        
        if print_details:
            print(f"\n📊 GP 推荐最佳点:")
            print(f"   参数: {gp_best_params}")
            print(f"   采集函数值（近似）: {gp_best_acq_value:.6f}")
        ...
        for llm_params in llm_suggestions:
            try:
                ...
                if print_details:
                    print(f"\n   LLM 推荐点: {llm_params}")
                    print(f"   采集函数值（近似）: {acq_value:.6f}")
                    print(f"   与最佳点差值: {acq_diff:.6f}")
                    print(f"   阈值: {self.acquisition_threshold}")
                    print(f"   是否接受: {'✅' if acq_diff < self.acquisition_threshold else '❌'}")
        ...
        for llm_suggestion in evaluated_llm_suggestions:
            if llm_suggestion["acquisition_diff"] < self.acquisition_threshold:
                accepted_suggestions.append(llm_suggestion["parameters"])
