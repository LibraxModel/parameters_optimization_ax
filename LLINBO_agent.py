...
class ParameterSpace:
    ...
    is_ordered: Optional[bool] = None  # 是否有序（仅用于choice类型）
...
class LLINBOAgent:
    ...
    def _build_parameter_space_prompt(self) -> str:
        ...
        for i, param in enumerate(self.parameter_space, 1):
            ...
            if param.type == "range" and param.bounds:
                ...
                param_desc.append(f"   类型: 离散参数")
                param_desc.append(f"   可选值: {param.values}")
    ...
    def _build_prior_data_prompt(self) -> str:
        ...
        # 添加数据统计信息
        if len(self.prior_experiments) > 0:
            prompt_parts.append("\n## 数据统计")
            for metric_name in self.objectives.keys():
                metric_values = [exp.metrics.get(metric_name) for exp in self.prior_experiments 
                               if metric_name in exp.metrics]
                if metric_values:
                    prompt_parts.append(
                        f"- {metric_name}: 最小值={min(metric_values):.4f}, "
                        f"最大值={max(metric_values):.4f}, "
                        f"平均值={np.mean(metric_values):.4f}"
                    )
    ...
    def _build_optimization_prompt(self, num_suggestions: int = 1) -> str:
        ...
        prompt_parts = [
            ...
            "1. 参数值必须在定义的范围内",
            "2. 考虑先验数据中的模式和趋势",
            "3. 在探索（exploration）和利用（exploitation）之间取得平衡",
            "4. 如果存在多个目标，需要考虑多目标优化（帕累托最优）",
            "",
    ...
    def _parse_llm_response(self, response: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "suggestions" in result:
                ...
                # 如果结果是单个字典，检查是否包含参数
                if any(key in result for key in [p.name for p in self.parameter_space]):
                    return [result]
                else:
                    return []
            else:
    ...
    def suggest_parameters(self, num_suggestions: int = 1) -> List[Dict[str, Any]]:
        """
        生成参数优化建议
    ...
    def get_optimization_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_experiments": len(self.prior_experiments),
            "parameter_space_size": len(self.parameter_space),
            "objectives": list(self.objectives.keys()),
            "problem_context": {
                "industry": self.problem_context.industry,
                "description": self.problem_context.problem_description[:100] + "..."
            }
        }
