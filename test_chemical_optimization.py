...
def main():
    ...
    return suggestions, agent, prior_experiments
...
def run_llinbo_optimization(df, parameters, objectives, problem_context, 
    ...
    for round_num in range(2, n_rounds + 1):
        ...
        if len(all_experiments) > 0:
            recent_yields = [exp.metrics["yield"] for exp in all_experiments[-5:]]  # 显示最近5个实验的产率
            recent_costs = [exp.metrics["cost"] for exp in all_experiments[-5:]]  # 显示最近5个实验的成本
            print(f"   最近5个实验的产率: {[f'{y:.2f}' for y in recent_yields]}")
            print(f"   最近5个实验的成本: {[f'{c:.4f}' for c in recent_costs]}")
        ...
        agent = LLINBOAgent(
            ...
            objectives=objectives,
            llm_config=llm_config,
            prior_experiments=all_experiments.copy()  # 使用副本，确保数据正确传递
        )
        ...
        all_experiments.extend(round_prior_experiments)
        print(f"✅ 已将第 {round_num} 轮的 {len(round_prior_experiments)} 个实验结果加入到先验数据中")
...
if __name__ == "__main__":
    ...
    # 运行单次测试
    print("=" * 80)
    print("单次 LLINBO Agent 测试")
    print("=" * 80)
    suggestions, agent, prior_experiments = main()
    
    # 运行多轮优化测试（使用 LLINBO Agent 代替 update 接口）
    print("\n" + "=" * 80)
    print("多轮优化测试（使用 LLINBO Agent）")
    print("=" * 80)
